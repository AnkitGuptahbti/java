...
class Utils {
    ...
    // partition nums into 3 parts [smaller than pivot, equal to pivot, larger than
    // pivot]
    ...
    // Return false if next permutation is not available. (nums is not changed for
    // this case)
    ...
    // Return false if previous permutation is not available. (nums is not changed
    // for this case)
    ...
    // This function is deprecated, please hash(int l, int r) in class
    // StableStringHash or StringHash
...
class SuffixArray {
    ...
    private void sortIndex(int[] nums, int start, int end) {
        ...
        nums[pos] = nums[end];
        while (i < j) {
            while (i < j && s[nums[i]] < s[pivot])
                i++;
            if (i < j)
                nums[j--] = nums[i];

            while (i < j && s[pivot] < s[nums[j]])
                j--;
            if (i < j)
                nums[i++] = nums[j];
        }
    ...
    private void suffixSort(int[] s) {
        ...
        for (k = 1; k < n && rank[SA[n - 1]] < n - 1; k <<= 1) {
            ...
            for (nRank[SA[0]] = 0, i = 1; i < n; i++) {
                nRank[SA[i]] = (SA[i] + k >= n || SA[i - 1] + k >= n || rank[SA[i]] != rank[SA[i - 1]]
...
class Combination {
    ...
    public long P(int m, int n) {
        ...
        // return factorial[m] * inverse(factorial[m - n]) % MOD;
            ...
            return factorial[m] * inverseFactorial[m - n] % MOD;
    }
    ...
    public long C(int m, int n) {
        if (m == 0 || n == 0 || m == n)
            return 1;
        // return factorial[m] * inverse(factorial[m - n]) % MOD * inverse(factorial[n])
        // % MOD;
        else
            return factorial[m] * inverseFactorial[m - n] % MOD * inverseFactorial[n] % MOD;
    }
}
...
class Trie {
    ...
    public boolean contains(String word) {
        ...
        for (int i = 0; i < word.length(); i++) {
            int c = word.charAt(i) - 'a';
            if (cur.next[c] == null || cur.next[c].count <= 0L)
                return false;
            cur = cur.next[c];
...
class TrieBinary {
    ...
    public boolean contains(long v) {
        ...
        for (int i = numOfBits - 1; i >= 0; i--) {
            int b = ((v >>> i) & 1) == 0 ? 0 : 1;
            if (cur.next[b] == null || cur.next[b].count <= 0)
                return false;
            cur = cur.next[b];
        }
        return true;
...
class SegmentTree {
    ...
    private int query(int x, int l, int r, int queryL, int queryR) {
        ...
        if (queryR <= m)
            ...
            return query(x * 2 + 1, l, m, queryL, queryR) + query(x * 2 + 2, m + 1, r, queryL, queryR); //// TODO
    }
}

// Tested by Leetcode 732
// Lazy execution example is in
// https://leetcode.com/submissions/detail/1034182539/
...
class SparseSegmentTreeRangeSum {
    ...
    public SparseSegmentTreeRangeSum(long l, long r) {
        L = l;
        R = r;
        root = new TreeNode();
    }
    ...
    public long getSum(long queryL, long queryR) {
        return getSum(root, L, R, queryL, queryR);
    }

    // [queryL, queryR] is range of query, [L, R] are range of TreeNode.
    private long getSum(TreeNode root, long L, long R, long queryL, long queryR) {
        if (queryL > queryR || root == null)
            return 0L;
        if (queryL <= L && R <= queryR) {
            return root.sum;
        }
        long M = L + ((R - L) >> 1);
        if (queryR <= M) {
            return getSum(root.left, L, M, queryL, queryR);
        } else if (M + 1 <= queryL) {
            return getSum(root.right, M + 1, R, queryL, queryR);
        } else {
            return getSum(root.left, L, M, queryL, queryR) + getSum(root.right, M + 1, R, queryL, queryR);
        }
    }
}
...
class SparseSegmentTreeRangeMax {
    ...
    public SparseSegmentTreeRangeMax(long l, long r) { // [l, r] inclusive
        L = l;
        R = r;
        root = new TreeNode();
    }
    ...
    public void update(long position, long value) {
        update(root, L, R, position, value);
    }

    private void update(TreeNode root, long L, long R, long position, long value) {
        if (position < L || R < position)
            return;
        if (L == R) {
            root.max = value;
        ...
        if (root.right == null)
            root.right = new TreeNode();
        update(root.left, L, M, position, value);
        update(root.right, M + 1, R, position, value);
    ...
    public long get(long position) {
        ...
        return cur == null ? Long.MIN_VALUE : cur.max;
    }

    // Max[queryL...queryR] inclusive
    public long getMax(long queryL, long queryR) {
        return getMax(root, L, R, queryL, queryR);
    }

    // [queryL, queryR] is range of query, [L, R] are range of TreeNode.
    private long getMax(TreeNode root, long L, long R, long queryL, long queryR) {
        if (queryL > queryR || root == null)
        ...
        long M = L + ((R - L) >> 1);
        if (queryR <= M) {
            return getMax(root.left, L, M, queryL, queryR);
        } else if (M + 1 <= queryL) {
            return getMax(root.right, M + 1, R, queryL, queryR);
        } else {
            return Math.max(getMax(root.left, L, M, queryL, queryR), getMax(root.right, M + 1, R, queryL, queryR));
        }
    }
}
...
class RangeMax {
    ...
    long getMax(int l, int r) {
        ...
        for (l += n, r += n; l < r; l >>= 1, r >>= 1) {
            if ((l & 1) != 0)
                resl = Math.max(resl, t[l++]);
            if ((r & 1) != 0)
                resr = Math.max(t[--r], resr);
        }
...
class RangeMin {
    ...
    long getMin(int l, int r) {
        ...
        for (l += n, r += n; l < r; l >>= 1, r >>= 1) {
            if ((l & 1) != 0)
                resl = Math.min(resl, t[l++]);
            if ((r & 1) != 0)
                resr = Math.min(t[--r], resr);
        }
...
class SegmentTreeSet {
    ...
    public SegmentTreeSet(int low, int high, int maxSharedSize) {
        if (sharedBuffer == null) {
            int tn = 1;
            while (tn < maxSharedSize)
                tn <<= 1;
            sharedBuffer = new int[tn * 2];
        }
        start = low;
        int len = high - low + 1;
        n = 1;
    ...
    public int last() {
        int cur = 1;
        while (cur < n) {
            cur = (c[cur << 1 | 1] > 0) ? (cur << 1 | 1) : (cur << 1);
        }
        return cur - n + start;
    }

    public int lowerCount(int key) {
        int l = 0 + n, r = key - start + n, sum = 0; // [l, r)
        for (; l < r; l >>= 1, r >>= 1) {
            if ((l & 1) != 0)
                sum += c[l++];
            if ((r & 1) != 0)
    ...
    private T parse(String s, T defaultValue) {
        ...
        while (pos < s.length()) {
            ...
            if (sign == '+')
                st.push(cur);
            if (sign == '-')
                st.push(minus(defaultValue, cur));
            if (sign == '*')
                st.push(multiply(st.pop(), cur));
            if (sign == '/')
                st.push(divide(st.pop(), cur));
...
class EulerPath<T, E> {
    ...
    // undirected graph: all degree (number of edges for each node) are EVEN or only
    // one node has ODD degree (start node)
    // directed graph: all nodes have out-degree equal to in-degree, or only one
    // node has out-degree = in-degree + 1 (start node)
...
class DivideSticks {
    ...
    private static boolean dfs(int depth, int startIndex, int currentSum, int currentNumOfBlocks) {
        if (depth == nums.length)
            return currentNumOfBlocks == targetNumOfBlocks;
        if (currentNumOfBlocks >= targetNumOfBlocks)
            return false;
        if (nums[0] + currentSum > targetBlockSum)
            return false; // TODO
        ...
        for (int i = startIndex; i >= 0; i--) {
            ...
            if (nums[i] + currentSum > targetBlockSum)
                continue; // TODO
...
class RMQ { // NOTE!!! query return index, not value
    ...
    public RMQ(long[] input, boolean useMax) {
        ...
        for (int j = 1; (1 << j) <= n; j++)
            for (int i = 0; i + (1 << j) <= n; i++)
                if (useMax) {
                    d[j][i] = a[d[j - 1][i]] >= a[d[j - 1][i + (1 << (j - 1))]] ? d[j - 1][i]
                            : d[j - 1][i + (1 << (j - 1))];
                } else {
                    d[j][i] = a[d[j - 1][i]] <= a[d[j - 1][i + (1 << (j - 1))]] ? d[j - 1][i]
                            : d[j - 1][i + (1 << (j - 1))];
    ...
    public int query(int i, int j) { // NOTE!!! return index, not value
        int k = log2[j - i + 1];
        if (useMax)
            return a[d[k][i]] >= a[d[k][j - (1 << k) + 1]] ? d[k][i] : d[k][j - (1 << k) + 1];
        else
            return a[d[k][i]] <= a[d[k][j - (1 << k) + 1]] ? d[k][i] : d[k][j - (1 << k) + 1];
    }
}
...
class DigitDP {
    ...
    long dfs(StringBuilder curResult, String s, int isStart, int hasLimit, int pos, int odd, int even, int r) {
        if (pos == s.length()) {
            if (isStart == 1)
                return 0; // NOTE: special check for num == 0. TODO
        ...
        if (dp[isStart][hasLimit][pos][odd][even][r] != UNSET)
            return dp[isStart][hasLimit][pos][odd][even][r];
...
public class Solution {
    ...
    public long countPrefixSuffixPairs(String[] words) {
        ...
        for (String s : words) {
            StableStringHash hash = new StableStringHash(s);

            for (int i = 0; i < s.length(); i++) {
                long l = hash.hash(0, i);
                long r = hash.hash(s.length() - i - 1, s.length() - 1);
                if (l == r) {
                    ans += count.getOrDefault(l, 0);
                }
            }
            long t = hash.hash(0, s.length() - 1);
